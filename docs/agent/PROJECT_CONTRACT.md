...
T08 adds only one Project primary Feishu Task List binding, bounded task/subtask/member/comment/completion projection, internal normalized task-event ingestion, periodic full reconciliation, explicit outside-list references, and versioned task title/description/completion writes. It does not implement task creation/deletion, member assignment writes, comments writes, reference removal, OAuth callback/refresh ownership, Calendar binding, Docs/files, permission administration, a public webhook endpoint, or a general integration-adapter registry.
